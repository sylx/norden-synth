...
composer.snapshot(seq.position()!.bar)  // 今の小節の調・拍子・和音・伴奏セット・編成 (表示用)
```
...
- **メロディ**は 1 音ずつ選ぶのではなく、小節ごとに音型 (アルペジオ・音階の走句・クリシェ・線の下を和音で埋める分散クリシェ) を決めて和音に当てはめる。クリシェは和音が 2 小節以上続く西洋の旋法では半音で動く (8 → 7 → ♭7 → 6 など)。句は音型の並びと終止からなり、セクションは A (半終止) → A' (全終止) → B → A' と同じ音型を繰り返す。最初の A は主題としてあとのセクションでも再現する。強拍の長い音に前打音・モルデント・トリルを付ける
